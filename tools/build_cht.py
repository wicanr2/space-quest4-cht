...
def main():
    ...
    ap.add_argument("--prefix", default="sq4", help="輸出字型檔名前綴")
    ap.add_argument("--no-font", action="store_true",
                    help="只產 runtime translation.tsv,不烘 TTF 字型(SQ4 走倚天點陣,見 build_eten_font.py)")
    ...
    if a.no_font:
        print(f"譯文 {len(rows)} 則 → {runtime}(字型由 build_eten_font.py 烘)")
        return
    ...
    fnt = a.outdir + f"/{a.prefix}_big5.fnt"
