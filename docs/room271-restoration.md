...
`self.sel116(5)` 查清楚了——`sel116` 是 `callk 7`，也就是 `RespondsTo`；selector 5 是
`underBits`。`Feature`（class 122）的屬性表裡沒有 selector 5，所以 `RespondsTo` 為假，
物件進 **global 32**——跟原版 `building` 完全一樣。**登記路徑確認相同，不是這裡的問題。**

再往下的測試踩到一個坑，值得記下來：

> **用除錯器 `newRoom` 進場的房間，輸入是關著的。**
> 拿原版 613 當對照組時，進去之後游標一直是「Wait」，點什麼都沒反應——因為正常進場會跑
> 抵達腳本、腳本裡才會 HandsOn，而 `newRoom` 跳過了那一段。我的 271 是自己在 `init`
> 裡叫了 HandsOn 才有反應。
>
> 也就是說**先前所有「原版熱區點得到、我的點不到」的對照都不成立**——原版那邊根本沒開輸入。
> 這條路要重驗，得先想辦法在除錯器進場後把輸入打開。

### 端到端（用玩家的路徑進去）

改用真正的路徑測試：進時空艙、依序輸入三組座標。結果：

- 三組座標**都被遊戲接受**（每組送出後都跑出時空亂流的敘述）
- 第三組送出後還多出一句只在這條路徑出現的台詞：「而，終於……終於……」

序列判定顯然是活的。但 headless 連點在第三組不穩（讀數螢幕只進了一個符號就送出），
所以**最後那一步的換房沒能確認**。這段要靠真人玩一次，或把鍵盤輸入改成更可靠的注入方式。
