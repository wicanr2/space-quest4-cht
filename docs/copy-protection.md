...
**完整座標表已經整理好了**，包含每一個能去的地方、被剪掉的隱藏房間，以及為什麼有一半的座標
每個人都不一樣：[時空艙座標全解](timepod-coordinates.md)。
線上可以先試按：<https://wicanr2.github.io/space-quest4-cht/timepod/>

下面四張是社群整理的儀表板截圖（本專案未逐一實測，僅供對照）。
**先存檔**——有幾個地方會直接害死羅傑。
